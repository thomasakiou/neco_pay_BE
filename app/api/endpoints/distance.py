...
def list_distances(skip: int = 0, limit: int = 100000, repo: DistanceRepository = Depends(get_repository)):
    return repo.list(skip, limit)

@router.post("/", response_model=DistanceDTO)
def create_distance(dto: CreateDistanceDTO, repo: DistanceRepository = Depends(get_repository)):
    distance = Distance(
        id=None,
        created_at=None,
...
def get_distance(id: int, repo: DistanceRepository = Depends(get_repository)):
...
def update_distance(id: int, dto: UpdateDistanceDTO, repo: DistanceRepository = Depends(get_repository)):
    distance = Distance(
        id=id,
        created_at=None,
        **dto.dict(exclude_unset=True)
...
def delete_all_distances(repo: DistanceRepository = Depends(get_repository)):
    repo.delete_all()
    return

@router.delete("/{id}", status_code=204)
def delete_distance(id: int, repo: DistanceRepository = Depends(get_repository)):
...
@router.post("/upload")
async def upload_distances(file: UploadFile = File(...), service: DistanceService = Depends(get_service)):
