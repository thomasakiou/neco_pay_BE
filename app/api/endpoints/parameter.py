...
def list_parameters(skip: int = 0, limit: int = 100000, repo: ParameterRepository = Depends(get_repository)):
    return repo.list(skip, limit)

@router.post("/", response_model=ParameterDTO)
def create_parameter(dto: CreateParameterDTO, repo: ParameterRepository = Depends(get_repository)):
    parameter = Parameter(
        id=None,
        created_at=None,
...
def get_parameter(id: int, repo: ParameterRepository = Depends(get_repository)):
...
def update_parameter(id: int, dto: UpdateParameterDTO, repo: ParameterRepository = Depends(get_repository)):
    parameter = Parameter(
        id=id,
        created_at=None,
        **dto.dict(exclude_unset=True)
...
def delete_all_parameters(repo: ParameterRepository = Depends(get_repository)):
    repo.delete_all()
    return

@router.delete("/{id}", status_code=204)
def delete_parameter(id: int, repo: ParameterRepository = Depends(get_repository)):
...
@router.post("/upload")
async def upload_parameters(file: UploadFile = File(...), service: ParameterService = Depends(get_service)):
